...
#include "Common.cginc"
	...
	float4 RotateAroundYInDegrees(float4 vertex, float degrees)
	{
		...
		float2x2 m = float2x2(cosa, -sina, sina, cosa); //回転行列
	...
	void vert(inout appdata v)
	{
		...
		//変換行列
		matrix_._11_22_33_44 = float4(_CubeMeshScale.xyz, 1.0);//scale transform
		matrix_._14_24_34 += _CubeDataBuffer[unity_InstanceID].Position;
		
		//v.vertex = RotateAroundYInDegrees(v.vertex, 180*sin(_time));
		//v.vertex = mul(Euler4x4(_CubeDataBuffer[unity_InstanceID].Rotation), v.vertex);
		
		//v.vertex = mul(matrix_, v.vertex+ float4(0.0 , 50.0*sin(_time+unity_InstanceID), 0.0, 1.0));
		v.vertex = mul(matrix_, v.vertex + float4(float3(0.0, 0.0, 0.0), 1.0));
